...
namespace rlab {
    ...
    export class EventHandler {// extends Array<{ (sender: any, event: any): void;}> {
        handlers: any[] = [];
        ...
        public fire(sender: any, event: any): void {
    ...
    export class TimeLine {
        ...
        dateRangeChanged: EventHandler = new EventHandler();
        selectedItemsChanged: EventHandler = new EventHandler();
        ...
        protected CreateView(options: TimeLineOptions) {
            ...
            for (var c = 0; c < this.options.panels.length && c < elements.length; c++) {
                ...
                var panel: TimeLinePanel = {
                    ...
                    fields: [],
                    legendPanel: <HTMLElement>element.getElementsByClassName("timeline_legend_panel").item(0) ,
                    axisPanel: <HTMLElement>element.getElementsByClassName("timeline_axis_panel").item(0),
                    canvasPanel: <HTMLElement>element.getElementsByClassName("timeline_canvas_panel").item(0),
                    canvas: <HTMLCanvasElement>element.getElementsByClassName("timeline_canvas").item(0),
                    grid: <HTMLElement>element.getElementsByClassName("timeline_grid").item(0)
                };
                ...
                for (var l = 0; l < legendPanel.length; l++) {
                    var legend = <HTMLElement>legendPanel.item(l);
            ...
            this.scales = [];
            let scales = this.root.getElementsByClassName("timeline_scale");
            for (var c = 0; c < scales.length; c++) {
                this.scales.push(<HTMLElement>scales[c]);
                //this.AddEventListener(<HTMLElement>scales[c], "mousewheel", this.OnMouseWheel.bind(this));

            }
            this.grids = [];
            let grids = this.root.getElementsByClassName("timeline_grid");
            for (var c = 0; c < grids.length; c++) {
                this.grids.push(<HTMLElement>grids[c]);
            }
        ...
        public AddPanel(panel: TimeLinePanel) {
            if (this.panels.indexOf(panel) == -1) {

            }
        }

        public RemovePanel(panel: TimeLinePanel) {

        }

        private OnLegendClick(element: HTMLElement, field: TimeLineField,  event: MouseEvent) {
            field.IsVisible = !field.IsVisible;
            element.style.textDecoration = (field.IsVisible ? "none" : "line-through");
            this.Draw("OnLegendClick");
        }

        private OnClick(event: MouseEvent = <MouseEvent>window.event) {
            if (!this.dragData.isMove) {
                if (this.isSelectTime || this.isSelectItems) {
                    var canvas = <HTMLCanvasElement>event.currentTarget;
        ...
        private OnMouseDown(event: MouseEvent = <MouseEvent>window.event) {
            var canvas = <HTMLCanvasElement>event.currentTarget;
        ...
        private OnMouseMove(event: MouseEvent = <MouseEvent>window.event) {
            if (this.isMoveHorizontal || this.isMoveVertical) {
        ...
        private OnMouseUp(event: MouseEvent = <MouseEvent>window.event) {
            if (this.isMoveHorizontal || this.isMoveVertical) {
                if (this.dragData.isBeginDrag) {
        ...
        private OnMouseWheel(event: WheelEvent = <WheelEvent>window.event) {
            if (this.isZoom) {
                var canvas = <HTMLCanvasElement>event.currentTarget;
                var delta = 0;
                if ((<any>event).wheelDelta) { /* IE/Opera. */
                    delta = (<any>event).wheelDelta / 120;
        ...
        protected ArrangeCanvas(panel: TimeLinePanel): void {
        ...
        protected DrawScale(): void {
            if (this.scales.length > 0) {
                let minorLabelPanel: HTMLElement = document.createElement("DIV");
                ...
                let majorLabelPanel: HTMLElement = document.createElement("DIV");
                ...
                let gridPanel: HTMLElement[] = [];

                // calculate minimum step (in milliseconds) based on character size
                let minimumStep = this.ScreenToTime((this.scaleCharSize.Width - 3) * 6).valueOf() - this.ScreenToTime(0).valueOf();
                this.hStep.setRange(this.begin, this.end, minimumStep);

                //// create a left major label
                var leftMajorLabel: HTMLElement = document.createElement("DIV");
            ...
            for (var c = 0; c < this.panels.length; c++) {
                ...
                if (panel.isAxis) {
                    ...
                    for (var d = 0; d < axisTextPanel.childNodes.length; d++) {
                        maxWidth = Math.max(maxWidth, (<HTMLElement>axisTextPanel.childNodes[d]).offsetWidth);
                    }
        ...
        public TimeToScreen(date: Date): number {
            return (date.valueOf() - this.begin.valueOf()) / this.GetTimeScale();
        }

        public ScreenToTime(x: number): Date {
        ...
        public SetTimeScale(scale: number, scaleAroundDate: Date = undefined): void {
        ...
        public ValueToScreen(panel: TimeLinePanel, value: number): number {
            //return (panel.max - value) / this.GetValueScale(panel);
            return (panel.canvas.height - 2) - ((value - panel.min) / this.GetValueScale(panel));
        }

        public ScreenToValue(panel: TimeLinePanel, y: number): number {
            //return panel.max - (value * this.GetValueScale(panel));
            return (((panel.canvas.height - 2) - y) * this.GetValueScale(panel)) + panel.min;
        }

        public GetValueScale(panel: TimeLinePanel): number {
            return (panel.max - panel.min) / (panel.canvas.height - 4);
        }

        public SetValueScale(panel: TimeLinePanel, scale: number, scaleAroundValue: number = undefined): void {
        ...
        private AddEventListener(element: any, action: string, listener: any, useCapture: boolean = false) {
        ...
        private RemoveEventListener(element: any, action: string, listener: any, useCapture: boolean = false) {
        ...
        private PreventDefault(event: Event = window.event) {
        ...
        defaultTemplate: string = '<table class="timeline_root">${PanelsTemplate}</table>';
        defaultTemplatePanel: string = '\
        ...
        defaultTemplateScale: string = '<tr><td></td><td class="timeline_scale"></td></tr>';

        defaultTemplateLegendItem: string = '\
    ...
    class StepDate {
        ...
        setMinimumStep(minimumStep: number) {
            ...
            if (stepYear * 1000 > minimumStep)      { this.scale = Scale.YEAR;        this.step = 1000; }
            if (stepYear * 500 > minimumStep)       { this.scale = Scale.YEAR;        this.step = 500; }
            if (stepYear * 100 > minimumStep)       { this.scale = Scale.YEAR;        this.step = 100; }
            if (stepYear * 50 > minimumStep)        { this.scale = Scale.YEAR;        this.step = 50; }
            if (stepYear * 10 > minimumStep)        { this.scale = Scale.YEAR;        this.step = 10; }
            if (stepYear * 5 > minimumStep)         { this.scale = Scale.YEAR;        this.step = 5; }
            if (stepYear > minimumStep)             { this.scale = Scale.YEAR;        this.step = 1; }
            if (stepMonth * 3 > minimumStep)        { this.scale = Scale.MONTH;       this.step = 3; }
            if (stepMonth > minimumStep)            { this.scale = Scale.MONTH;       this.step = 1; }
            if (stepDay * 5 > minimumStep)          { this.scale = Scale.DAY;         this.step = 5; }
            if (stepDay * 2 > minimumStep)          { this.scale = Scale.DAY;         this.step = 2; }
            if (stepDay > minimumStep)              { this.scale = Scale.DAY;         this.step = 1; }
            if (stepDay / 2 > minimumStep)          { this.scale = Scale.WEEKDAY;     this.step = 1; }
            if (stepHour * 4 > minimumStep)         { this.scale = Scale.HOUR;        this.step = 4; }
            if (stepHour * 3 > minimumStep)         { this.scale = Scale.HOUR;        this.step = 3; }
            if (stepHour > minimumStep)             { this.scale = Scale.HOUR;        this.step = 1; }
            if (stepMinute * 15 > minimumStep)      { this.scale = Scale.MINUTE;      this.step = 15; }
            if (stepMinute * 10 > minimumStep)      { this.scale = Scale.MINUTE;      this.step = 10; }
            if (stepMinute * 5 > minimumStep)       { this.scale = Scale.MINUTE;      this.step = 5; }
            if (stepMinute > minimumStep)           { this.scale = Scale.MINUTE;      this.step = 1; }
            if (stepSecond * 15 > minimumStep)      { this.scale = Scale.SECOND;      this.step = 15; }
            if (stepSecond * 10 > minimumStep)      { this.scale = Scale.SECOND;      this.step = 10; }
            if (stepSecond * 5 > minimumStep)       { this.scale = Scale.SECOND;      this.step = 5; }
            if (stepSecond > minimumStep)           { this.scale = Scale.SECOND;      this.step = 1; }
            if (stepMillisecond * 200 > minimumStep){ this.scale = Scale.MILLISECOND; this.step = 200; }
            if (stepMillisecond * 100 > minimumStep){ this.scale = Scale.MILLISECOND; this.step = 100; }
            if (stepMillisecond * 50 > minimumStep) { this.scale = Scale.MILLISECOND; this.step = 50; }
            if (stepMillisecond * 10 > minimumStep) { this.scale = Scale.MILLISECOND; this.step = 10; }
            if (stepMillisecond * 5 > minimumStep)  { this.scale = Scale.MILLISECOND; this.step = 5; }
            if (stepMillisecond > minimumStep)      { this.scale = Scale.MILLISECOND; this.step = 1; }
        };

        getLabelMajor(date: Date = undefined) {
            ...
            switch (this.scale) {
                ...
                case Scale.MILLISECOND: return (<any>date).format("dd mmmm HH:MM:ss", true); //`0${date.getUTCHours()}`.slice(-2) + ":" + `0${date.getUTCMinutes()}`.slice(-2) + ":" + `0${date.getUTCSeconds()}`.slice(-2);
                case Scale.SECOND:      return (<any>date).format("dd mmmm HH:MM", true); //`${date.getUTCDate()} ${MONTHS[date.getUTCMonth()]} ${this.addZeros(date.getUTCHours(), 2)}:${this.addZeros(date.getUTCMinutes(), 2)}`;
                case Scale.MINUTE:      return (<any>date).format("dddd dd mmmm yyyy", true); //`${DAYS[date.getUTCDay()]} ${date.getUTCDate()} ${MONTHS[date.getUTCMonth()]} ${date.getUTCFullYear()}`;
                case Scale.HOUR:        return (<any>date).format("dddd dd mmmm yyyy", true); //`${DAYS[date.getUTCDay()]} ${date.getUTCDate()} ${MONTHS[date.getUTCMonth()]} ${date.getUTCFullYear()}`;
                case Scale.WEEKDAY:
                case Scale.DAY:         return (<any>date).format("mmmm yyyy", true); //`${MONTHS[date.getUTCMonth()]} ${date.getUTCFullYear()}`;
                case Scale.MONTH:       return (<any>date).format("yyyy", true); //`${date.getUTCFullYear()}`;
                default:                return "";
            }
        }

        getLabelMinor(date: Date = undefined) {
            ...
            switch (this.scale) {
                case Scale.MILLISECOND: return (<any>date).format("l", true); //String(date.getUTCMilliseconds());
                case Scale.SECOND:      return (<any>date).format("ss", true); //this.addZeros(String(date.getUTCSeconds()), 2);
                case Scale.MINUTE:      return (<any>date).format("HH:MM", true); //this.addZeros(date.getUTCHours(), 2) + ":" + this.addZeros(date.getUTCMinutes(), 2);
                case Scale.HOUR:        return (<any>date).format("HH:MM", true); //this.addZeros(date.getUTCHours(), 2) + ":" + this.addZeros(date.getUTCMinutes(), 2);
                case Scale.WEEKDAY:     return (<any>date).format("ddd dd", true); //DAYS_SHORT[date.getUTCDay()] + ' ' + date.getUTCDate();
                case Scale.DAY:         return (<any>date).format("dd", true); //this.addZeros(String(date.getUTCDate()), 2);
                case Scale.MONTH:       return (<any>date).format("mmm", true); //MONTHS_SHORT[date.getUTCMonth()];   // month is zero based
                case Scale.YEAR:        return (<any>date).format("yyyy", true); //String(date.getUTCFullYear());
                default:                return "";
            }
        };
        ...
        start() {
            ...
            switch (this.scale) {
                case Scale.YEAR:   this.current.setUTCFullYear(this.step * Math.floor(this.current.getUTCFullYear() / this.step));
            ...
            if (this.step != 1) {
                ...
                switch (this.scale) {
                    case Scale.MILLISECOND:this.current.setMilliseconds(this.current.getMilliseconds() - this.current.getMilliseconds() % this.step); break;
                    case Scale.SECOND:     this.current.setSeconds(this.current.getSeconds() - this.current.getSeconds() % this.step); break;
                    case Scale.MINUTE:     this.current.setMinutes(this.current.getMinutes() - this.current.getMinutes() % this.step); break;
                  //case Scale.HOUR:       this.current.setHours(this.current.getHours() - this.current.getHours() % this.step); break;
                    case Scale.HOUR:       this.current.setUTCHours(this.current.getUTCHours() - this.current.getUTCHours() % this.step); break;
                    case Scale.WEEKDAY:    // intentional fall through
                    case Scale.DAY:        this.current.setDate((this.current.getDate() - 1) - (this.current.getDate() - 1) % this.step + 1); break;
                    case Scale.MONTH:      this.current.setMonth(this.current.getMonth() - this.current.getMonth() % this.step); break;
                    case Scale.YEAR:       this.current.setFullYear(this.current.getFullYear() - this.current.getFullYear() % this.step); break;
                    default: break;
                }
            }
        };
        ...
        next() {
            ...
            //else {
                switch (this.scale) {
                    case Scale.MILLISECOND:this.current = new Date(this.current.valueOf() + this.step); break;
                    case Scale.SECOND:     this.current.setSeconds(this.current.getSeconds() + this.step); break;
                    case Scale.MINUTE:     this.current.setMinutes(this.current.getMinutes() + this.step); break;
                    case Scale.HOUR:       this.current.setUTCHours(this.current.getUTCHours() + this.step);
                        //this.current.setUTCHours(this.current.getUTCHours() - this.current.getUTCHours() % this.step);
                        //console.log("2");
                        break;
                    case Scale.WEEKDAY:    // intentional fall through
                    case Scale.DAY:        this.current.setDate(this.current.getDate() + this.step); break;
                    case Scale.MONTH:      this.current.setMonth(this.current.getMonth() + this.step); break;
                    case Scale.YEAR:       this.current.setFullYear(this.current.getFullYear() + this.step); break;
                    default: break;
                }
            //}
            ...
            if (this.step != 1) {
                ...
                switch (this.scale) {
                    case Scale.MILLISECOND:if (this.current.getMilliseconds() < this.step) this.current.setMilliseconds(0); break;
                    case Scale.SECOND:     if (this.current.getSeconds() < this.step) this.current.setSeconds(0); break;
                    case Scale.MINUTE:     if (this.current.getMinutes() < this.step) this.current.setMinutes(0); break;
                    case Scale.HOUR:       if (this.current.getUTCHours() < this.step) this.current.setUTCHours(0); break;
                    case Scale.WEEKDAY:    // intentional fall through
                    case Scale.DAY:        if (this.current.getDate() < this.step + 1) this.current.setDate(1); break;
                    case Scale.MONTH:      if (this.current.getMonth() < this.step) this.current.setMonth(0); break;
    ...
    class StepNumber {
        ...
        constructor(start?: number, end?: number, step?: number, prettyStep?: boolean) {
            this._start = 0;
            this._end = 0;
            this._step = 1;
        ...
        _setRange(start?: number, end?: number, step?: number, prettyStep?: boolean) {
        ...
        setStep = function (step?: number, prettyStep?: boolean) {
        ...
        _calculatePrettyStep(step?: number) {
        ...
        ko.components.register("timeline", {
            viewModel: {
                createViewModel: function (params, componentInfo) {
                    ...
                    var root = <HTMLElement>componentInfo.element;
                    ...
                    for (var p = 0; p < TLParams.panels.length; p++) {
                        ...
                        let __fields: TimeLineField[] = ko.utils.unwrapObservable(TLParams.panels[p].fields);
                        for (var f = 0; f < __fields.length; f++) {
                            var field: TimeLineField = {
                    ...
                    var timeline = new TimeLine(root, <TimeLineOptions>options);
                    ...
                    for (var p = 0; p < TLParams.panels.length; p++) {
                        ...
                        if (ko.isObservable(TLParams.panels[p].fields)) {
                            ...
                            for (var f = 0; f < TLParams.panels[indxP].fields.length; f++) {
                                if (TLParams.panels[indxP].fields[f].data && ko.isObservable(TLParams.panels[indxP].fields[f].data)) {
                                    let indxF = f;
                                    TLParams.panels[indxP].fields[f].data.subscribe(function (newValue) {
                                        //console.log(indxP + " : " + indxF);
                                        timeline.panels[indxP].fields[indxF].data = newValue;
                                        timeline.Draw("field");
                                    });
                                }
                            }
